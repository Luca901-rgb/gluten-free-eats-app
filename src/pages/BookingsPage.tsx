...
import { Calendar, Clock, MapPin, Users, MessageSquare, Trash2, Copy } from 'lucide-react';
import {
  Card,
  CardContent,
  CardDescription,
...
const BookingsPage = () => {
  ...
  const navigateToRestaurantReviews = (restaurantId: string, bookingCode: string, restaurantCode?: string) => {
    let url = `/restaurant/${restaurantId}?tab=reviews&bookingCode=${bookingCode}`;
    if (restaurantCode) {
      url += `&restaurantCode=${restaurantCode}`;
    }
    navigate(url);
  };
  ...
  const BookingCard = ({ booking }: { booking: any }) => {
    ...
    const canBeReviewed = isCompleted && !booking.hasReview && booking.attendance === 'confirmed';
      ...
      <Card className="overflow-hidden animate-fade-in">
        ...
        <CardFooter className="pt-0 flex flex-col gap-2">
          ...
          {canBeReviewed && (
            <Button 
              className="w-full" 
              onClick={() => navigateToRestaurantReviews(
                booking.restaurantId, 
                booking.bookingCode, 
                booking.restaurantReviewCode
              )}
            >
              Visita ristorante per recensione
            </Button>
          )}
          
          {isCompleted && booking.attendance !== 'confirmed' && (
